...
#define UI_USER_DEFINED_COMMAND( CommandPtr, CommandId, FriendlyName, InDescription, CommandType, InDefaultChord, ... ) \
	MakeUICommand_InternalUseOnly( this, CommandPtr, TEXT(LOCTEXT_NAMESPACE), *CommandId, *(CommandId + TEXT("_ToolTip")), TCHAR_TO_ANSI(*("." + CommandId)), *FriendlyName, TEXT(InDescription), CommandType, InDefaultChord, ## __VA_ARGS__ );
